...
function onLoad()
    ...
    qigong_bag = Global.getVar('qigong_bag')
    params = {
...
function click_func(obj, color, alt_click)
    ...
    local bag = getObjectFromGUID(qigong_bag)
    local public_zone = getObjectFromGUID(public_pile_guid)
    if bag.getQuantity() == 1 then
        Global.call('addNewStyle', qigong_bag)
        broadcastToAll('Select qigong style!', {r=0, g=1, b=0})
        local params = {

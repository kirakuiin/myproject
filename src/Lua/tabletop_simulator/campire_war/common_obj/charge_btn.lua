...
function onLoad()
    ...
    charge_bag = Global.getVar('charge_bag')
    params = {
...
function click_func(obj, color, alt_click)
    ...
    local bag = getObjectFromGUID(charge_bag)
    local public_zone = getObjectFromGUID(public_pile_guid)
    if bag.getQuantity() == 1 then
        Global.call('addNewStyle', charge_bag)
        broadcastToAll('Select charge style!', {r=0, g=1, b=0})
        local params = {

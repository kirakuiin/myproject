#+TAGS: 位操作(b) 递归(r) 回溯(j) 动态规划(d) 贪心(e) 排序(o) 深度优先搜索(l) 广度优先搜索(f) 启发式(i) 指针(p) 二分(k) 分治(c) 概率(t)
#+TAGS: 并查集(u) 栈(s) 图(g) 数学(m) 序列(a) 堆(h) 队列(q)
...
** 霍夫曼编码                                                         :贪心:
*** 情景
实现一种高效的变长前缀编码，可以将文本转换为二进制数据，也可以通过某种手段
还原回来。
*** 解法

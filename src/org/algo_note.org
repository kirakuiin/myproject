...
#+TAGS: 并查集(u) 栈(s) 图(g) 数学(m) 序列(a) 堆(h) 队列(q) 树(t) 数据结构(z)
...
** 满足快速增删改查序列数据结构                                    :数据结构:
*** 情景
    设计一种数据结构，能够在O(lgn)的时间复杂度内完成增删改查。
*** 解法
**** 跳跃表                                                       :概率:序列:
     常见的数组满足O(1)时间内的查询，但是增删需要O(n)复杂度，链表可以实现O(1)的增删，
     但是查询需要O(n), 为了给链表的查询增速，可以利用二分的思想，额外加一层链表，数量
     底层链表的一半，这样查询的速度变为O(n/2), 而新加的这层也可以利用这个思想增速查询，
     最后经过多层链表查询的速度为O(lgn)。
     这样的数据结构如何来决定哪个节点有二层，三层链表呢？常用的策略是投掷硬币法，每插入
     一个节点，投一次币来决定他是否有下一级链表。我们假设最底层为高度为1，高度为2的链表
     概率为50%，高度为3的链表概率为25%，以此类推，因此用这种方法可以符合二分的定义。
**** TODO 红黑树                                                         :树:
*** 伪代码
**** TODO 跳跃表                                                  :概率:序列:
**** TODO 红黑树                                                         :树:

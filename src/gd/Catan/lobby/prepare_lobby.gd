...
remotesync func change_max_player_num(index: int):
    $OptionContainer/PlayerNumContainer/PlayerNumBtn.select(index)
    _host_info.max_player_num = Data.max_player_data[index]


remotesync func change_fog_state(is_pressed: bool):
    $OptionContainer/FogContainer/FogCheckBtn.pressed = is_pressed
...
func _on_change_fog(button_pressed):
    rpc("change_fog_state", button_pressed)

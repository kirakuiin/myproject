...
inline void processInput(GLFWwindow *window, float delta, gl::ShaderProgram& p)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) {
        p.Use();
        p.SetUniform("ispara", 0);
    }
    if (glfwGetKey(window, GLFW_KEY_P) == GLFW_RELEASE) {
        p.Use();
        p.SetUniform("ispara", 1);
    }
}
...
class PAMUT : public testing::Test {
  public:
...
TEST_F(PAMUT, HelloParallaxMap) {
    ...
    // set shader
    gl::VertexShader v_shader("shaders/parallaxmap/shader.vert");
    gl::FragmentShader f_shader("shaders/parallaxmap/shader.frag");
    ...
    // -------------
    // gl::Texture2D brick;
    // brick.LoadImage("images/texture/toy_box.png");
    // gl::Texture2D normal;
    // normal.LoadImage("images/texture/toy_box_normal.png");
    // gl::Texture2D disp;
    // disp.LoadImage("images/texture/toy_box_disp.png");
    gl::Texture2D brick;
    brick.LoadImage("images/texture/bricks2.jpg");
    gl::Texture2D normal;
    normal.LoadImage("images/texture/bricks2_normal.jpg");
    gl::Texture2D disp;
    disp.LoadImage("images/texture/bricks2_disp.jpg");
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, brick.texture);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, normal.texture);
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, disp.texture);
    program.Use();
    program.SetUniform("brick", 0);
    program.SetUniform("normal", 1);
    program.SetUniform("depth", 2);
    program.SetUniform("height_scale", 0.1f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // input
        // -----
        processInput(window, deltaTime, program);

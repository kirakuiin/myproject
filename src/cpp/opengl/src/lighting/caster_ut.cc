...
class CASUT : public testing::Test {
  public:
...
TEST_F(CASUT, HelloCaster) {
    ...
    // set shader
    gl::VertexShader v_shader("shaders/caster/vertex_shader.gls");
    gl::FragmentShader of_shader("shaders/caster/of_shader.gls");
    gl::FragmentShader lf_shader("shaders/caster/lf_shader.gls");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        obj_program.SetUniform("light.position", vec4(0, 0, 0, 1));
        ...
        obj_program.SetUniform("light.direct", view * vec4(camera.front, 0));
        obj_program.SetUniform("light.cutoff", glm::cos(glm::radians(12.5f)));
        obj_program.SetUniform("light.outoff", glm::cos(glm::radians(20.0f)));
        obj_program.SetUniform("light.constant", 1.0f);
        obj_program.SetUniform("light.linear", 0.22f);
        obj_program.SetUniform("light.quadratic", 0.0019f);

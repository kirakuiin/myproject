...
TEST_F(COLUT, HelloColor) {
    ...
    std::vector<float> obj_color{1.0, 0.5f, 0.31f};
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(float)*3, (void*)0);
    glEnableVertexAttribArray(0);
    std::vector<float> light_color{1.0, 1.0f, 1.0f};
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        light_program.SetUniform("model",
                                 gl::UniformType::MATRIX4,
                                 glm::value_ptr(model));
        light_program.SetUniform("view",
                                 gl::UniformType::MATRIX4,
        ...
        projection = glm::perspective(camera.zoom,
                                      ...
                                      0.1f, 100.0f);
        light_program.SetUniform("projection",
                                  gl::UniformType::MATRIX4,
        ...
        obj_program.SetUniform("model",
                               gl::UniformType::MATRIX4,
                               glm::value_ptr(model));
        obj_program.SetUniform("projection",
                                  gl::UniformType::MATRIX4,
                                  glm::value_ptr(projection));
        obj_program.SetUniform("view",
                                 gl::UniformType::MATRIX4,
                                 glm::value_ptr(camera.GetViewMatrix()));
        glBindVertexArray(vao[0]);
        glDrawArrays(GL_TRIANGLES, 0, 36);
